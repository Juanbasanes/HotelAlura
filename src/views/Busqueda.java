...
public class Busqueda extends JFrame {
    ...
    private void LlenarTablaReservas() {
        ...
        try {
            for (Reserva reservas : reserva) {
                modelo.addRow(
                        new Object[] { reservas.getId(), reservas.getfechaEntrada(), reservas.getfechaSalida(),
        ...
        try {
            for (Huespedes huespedes : huesped) {
                modeloHuesped.addRow(
                        new Object[] { huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(),
    ...
    private void ActualizarReservas() {
        try {
            ...
            if (filaReservas >= 0) {
                ...
                // Actualizar Reservas
                reservaController.actualizar(fechaEntrada, fechaSalida, valor, formaPago, id);
                JOptionPane.showMessageDialog(this, "Registro modificado con éxito");
                
            } else {
                JOptionPane.showMessageDialog(this, "Por favor, seleccione un registro para editar");
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Error al modificar el registro: " + e.getMessage());
            e.printStackTrace(); 
        }
    }
    ...
    private void ActualizarHuesped() {
        try {
            ...
            if (filaHuesped >= 0) {
                String nombre = (String) modeloHuesped.getValueAt(filaHuesped, 1);
                String apellido = (String) modeloHuesped.getValueAt(filaHuesped, 2);
                Date fechaNacimiento = Date.valueOf(modeloHuesped.getValueAt(filaHuesped, 3).toString());
                String nacionalidad = (String) modeloHuesped.getValueAt(filaHuesped, 4);
                String telefono = (String) modeloHuesped.getValueAt(filaHuesped, 5);
                Integer idReserva = Integer.valueOf(modeloHuesped.getValueAt(filaHuesped, 6).toString());
                Integer id = Integer.valueOf(modeloHuesped.getValueAt(filaHuesped, 0).toString());

                // Actualizar Huesped
                huespedesController.actualizar(nombre, apellido, fechaNacimiento, nacionalidad, telefono, idReserva, id);
                JOptionPane.showMessageDialog(this, "Registro modificado con éxito");
            } else {
                JOptionPane.showMessageDialog(this, "Por favor, seleccione un registro para editar");
            }
